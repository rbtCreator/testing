...
sys.path.insert(0, curdir + '/setup_data')
...
class AdviceWindow(QtWidgets.QWidget, adv.Ui_Form):
    ...
    def writeData(self):
        with open(os.path.join(os.path.abspath(os.curdir), "setup_data", "adv.csv"), 'w', newline='') as csvfile:
            fieldnames = ["adv", "phr"]
...
class InitWindow(QtWidgets.QWidget, test_create.Ui_Form):
    def __init__(self, finishFunc=None):
        ...
        self.img = QtGui.QImage(os.path.abspath(os.curdir) + "/setup_data/img1.jpeg")
        self.palette = QtGui.QPalette()
        self.palette.setBrush(QtGui.QPalette.Window, QtGui.QBrush(self.img))
        self.setPalette(self.palette)
    ...
    def data_output(self):
        ...
        with open(os.path.join(os.path.abspath(os.curdir), "test_data", "source.csv"), 'w', newline='') as csvfile:
